  ...
  return (
    <SafeAreaView className="flex-1 bg-white">
      <View className="border-border flex-row items-center border-b px-4 py-3">
        ...
        <Text className="text-foreground text-xl font-semibold">All Services</Text>
      </View>
      <FlatList
        data={services}
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('ServiceDetail', { serviceSlug: item.slug })}
            activeOpacity={0.7}>
            <Text className="text-foreground flex-1 text-lg">{item.name}</Text>
